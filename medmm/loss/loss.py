...
def nll_loss_new(logits, Y, c, alpha=0.0, eps=1e-7, reduction='mean'):
    """
    ...
    hazards = torch.sigmoid(logits[0])
...
class NLLSurvLoss_NEW(object):
    ...
    def __call__(self, logits, Y, c):
        ...
        # else:
        return nll_loss_new(logits, Y, c, alpha=self.alpha, eps=self.eps, reduction=self.reduction)
...
@LOSS_REGISTRY.register()
def nllsurvloss_new(**kwargs):
    return NLLSurvLoss_NEW(**kwargs)
